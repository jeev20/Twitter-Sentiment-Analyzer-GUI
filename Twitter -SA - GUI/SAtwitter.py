...
# Author - jeev20. https://github.com/jeev20
    ...
    # This program scraps twitter pages for tweets. Scraps the latest tweet and performs sentiment analysis using TextBlob package. 
...
root = Tk()
root.geometry("1200x450+300+400")
root.title("Text Sentiment Analyzer") 

# input of text in text input box
text = Text(root, width=10, height=10, wrap="word", font=("Helvetica",14), bg="#E5E5E5")
t = scrapper.getText()  # to get tweet as input
...
def help():
	output = " * Paste twitter account link in the terminal. \n  OR '\n' \n * Paste any word text for analysis."
	tkMessageBox.showinfo("About", output)
...
la0.configure(text =  scrapper.title(), font=("Helvetica",11))
la0.pack(side =TOP, padx=10)

la = Label(root, background="#CDDC39")
la.configure(text = scrapper.url, font=("Helvetica",11))
la.pack(side = TOP, padx=10 )

la1 = Label(root, background="#CDDC39")
la1.configure(text =  scrapper.time(), font=("Helvetica",11))
